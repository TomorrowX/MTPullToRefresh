...
import com.mingtian.mtpulltorefresh.internal.FlipLoadingLayout;
import com.mingtian.mtpulltorefresh.internal.LoadingLayout;
import com.mingtian.mtpulltorefresh.internal.RotateLoadingLayout;
import com.mingtian.mtpulltorefresh.internal.Utils;
import com.mingtian.mtpulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    /*是否开启上滑取消*/
    private boolean mEnablePullUpCancel = false;
    /*取消监听*/
    private OnRefreshCancelListener<T> mOnRefreshCancelListener;

    public void setEnablePullUpCancel(boolean mEnablePullUpCancel) {
        this.mEnablePullUpCancel = mEnablePullUpCancel;
    }

    public void setOnRefreshCancelListener(OnRefreshCancelListener<T> mOnRefreshCancelListener) {
        this.mOnRefreshCancelListener = mOnRefreshCancelListener;
    }
     ...
     * @return 是否处理
     */
    private boolean handlePullUpCancelWhenRefreshing(MotionEvent event) {
        //现在只支持垂直，下拉刷新取消
        if (!mEnablePullUpCancel || getPullToRefreshScrollDirection() != Orientation.VERTICAL ||
                mCurrentMode != Mode.PULL_FROM_START) {
            return true;
        }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mLastY - y > 0) {
                    mLastY = y;
                    int distance = (int) (mInitY - mLastY);
                    int headerSize = getHeaderSize();
                    pullEvent(mInitX, mLastX, 0, headerSize * 2 - distance);
                    //如果滑动已经超过了headerSize，就表示取消
                    if (distance > headerSize) {
                        if (mOnRefreshCancelListener != null) {
                            mOnRefreshCancelListener.onCanceled(this);
                        }
                        onRefreshComplete();
                    }
                }
            ...
            case MotionEvent.ACTION_UP: {
                int distance = (int) (mInitY - mLastY);
                int headerSize = getHeaderSize();
                //如果抬起时已经超过了headerSize / 2，就表示取消
                if (distance > headerSize / 2) {
                    if (mOnRefreshCancelListener != null) {
                        mOnRefreshCancelListener.onCanceled(this);
                    }
                    onRefreshComplete();
                } else {
                    pullEvent(mInitX, mLastX, 0, headerSize * 2);
                }
                break;
            }
        }
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        if (!mScrollingWhileRefreshingEnabled && isRefreshing()) {
            return handlePullUpCancelWhenRefreshing(event);
        }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE: {
                if (mIsBeingDragged) {
                    ...
                    pullEvent(mInitialMotionX, mLastMotionX, mInitialMotionY, mLastMotionY);
                    return true;
                }
                break;
            }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // Styleables from XML
        TypedArray a = context.obtainStyledAttributes(attrs, com.handmark.pulltorefresh.library.R.styleable.PullToRefresh);

        if (a.hasValue(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrMode)) {
            mMode = Mode.mapIntToValue(a.getInteger(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrMode, 0));
        }

        if (a.hasValue(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrAnimationStyle)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                    com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrAnimationStyle, 0));
        }
         ...
         * Styleables from XML
         */
        if (a.hasValue(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrRefreshableViewBackground)) {
            Drawable background = a.getDrawable(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrRefreshableViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a.hasValue(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrAdapterViewBackground)) {
            Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
            Drawable background = a.getDrawable(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrAdapterViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }

        if (a.hasValue(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrOverScroll)) {
            mOverScrollEnabled = a.getBoolean(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrOverScroll, true);
        }
        ...
        if (a.hasValue(com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrScrollingWhileRefreshingEnabled)) {
            mScrollingWhileRefreshingEnabled = a.getBoolean(
                    com.handmark.pulltorefresh.library.R.styleable.PullToRefresh_ptrScrollingWhileRefreshingEnabled, false);
        }
     ...
     * change
     */
    private void pullEvent(float initX, float lastX, float initY, float lastY) {
        ...
        switch (getPullToRefreshScrollDirection()) {
            case HORIZONTAL:
                initialMotionValue = initX;
                lastMotionValue = lastX;
                break;
            case VERTICAL:
            default:
                initialMotionValue = initY;
                lastMotionValue = lastY;
                break;
        }
        ...
        if (Math.abs(newScrollValue) <= Math.abs(itemDimension) * 2) {
    ...
    public interface OnRefreshCancelListener<V extends View> {

        /**
         * onRefresh will be called for both a Pull from start, and Pull from
         * end
         */
        void onCanceled(final PullToRefreshBase<V> refreshView);
